...
public class MongoLimitsDao implements LimitsDao {
    ...
    private void insertLimit(Promise<Void> resultPromise, long userId, long limit) {
        ...
        getCollection().insertOne(document)
                .subscribe(forVoidPromise(resultPromise, success -> true, new RuntimeException("Cannot insert limit")));
    }
    ...
    public Future<Optional<Long>> getLimitForUser(long userId) {
        ...
        getCollection()
                ...
                .first()
                .subscribe(forSinglePromise(resultPromise, doc -> Optional.of(doc.getLong("limit")), Optional.empty()));
        return resultPromise.future();
    }
